...
CDeviceEvent::CDeviceEvent(Signals Sig, DEV_HANDLE Device_H, const uint8_t* Data, uint16_t Len) : Event(Sig, true)
{
	this->m_Device = Device_H;
...
void CDeviceEvent::CopyData(uint8_t* Data, uint16_t Len)
{
	if (Len)
	{
		memcpy(this->m_Data, Data, Len);
				...
				MACCALLBACK MacCall, uint32_t DQSize)
{
	...
	m_IrqEvent = new  CDeviceEvent[2]{
		CDeviceEvent(SERIAL_RSP_SIG, m_Device, MaxFrameLen, false),
		CDeviceEvent(SERIAL_RSP_SIG, m_Device, MaxFrameLen, false)
	};
	ASSERT(m_IrqEvent);
	m_IrqEventIndex = 0;
...
void CDevKeeper::SendComplete(DEV_HANDLE DevHandle)
{
	...
	if (Device)
	{
		...
		Device->PostIrqEvent(SERIAL_HW_OUT_COMPLETE_SIG);
	}
}
...
void CDevKeeper::Receive(DEV_HANDLE DevHandle, uint8_t* Data, uint32_t Len)
{
	...
	if (Device && Device->m_MacCall(Device->m_Buff4MacCall, Device->m_BuffSize, Device->m_BuffCount, Data, Len))
	{
		...
		Device->PostIrqEvent(SERIAL_RSP_SIG);
	}
}
